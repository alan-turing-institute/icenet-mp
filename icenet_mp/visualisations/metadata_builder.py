...
class MetadataBuilder:
    """Extracts training metadata from Hydra configs and formats it for plot titles."""

    def extract_variables_by_source(  # noqa: C901
        ...
        try:
            ...
            logger.debug(
                "Failed to extract variables from config: %s", exc, exc_info=True
            )
    ...
    def calculate_training_points(
        self, start_str: str | None, end_str: str | None, cadence_str: str | None
    ) -> int | None:
    ...
    def format_cadence_display(self, cadence_str: str | None) -> str | None:
    ...
    def extract_cadence_from_config(self, config: dict[str, Any]) -> str | None:
        ...
        try:
            ...
            logger.debug(
                "Failed to extract cadence from config: %s", exc, exc_info=True
            )
        return None
    ...
    def extract_training_date_range(
        ...
        try:
            ...
            train_ranges = (
                split_cfg.get("train") if isinstance(split_cfg, dict) else None
            )
            ...
            logger.debug(
                "Failed to extract training date range from config: %s",
                exc,
                exc_info=True,
            )
    ...
    def build(
        ...
        start_str, end_str = self.extract_training_date_range(config)

        # Extract cadence
        cadence_str = self.extract_cadence_from_config(config)
        training_points = self.calculate_training_points(
    ...
    def format_subtitle(self, metadata: Metadata) -> str | None:  # noqa: C901, PLR0912
        ...
        if metadata.start or metadata.end:
            ...
            cadence_display = self.format_cadence_display(metadata.cadence)
