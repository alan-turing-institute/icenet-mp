...
from icenet_mp.visualisations.frame_renderer import FrameRenderer
...
class TestSafeLinspace:
    def test_normal_range(self) -> None:
        ...
        result = FrameRenderer()._safe_linspace(0.0, 1.0, 5)
    ...
    def test_equal_bounds_produce_a_tiny_range(self) -> None:
        ...
        result = FrameRenderer()._safe_linspace(2.0, 2.0, 3)
...
class TestPrepareStaticPlot:
    def test_rejects_mismatched_shapes(self) -> None:
        ...
        with pytest.raises(InvalidArrayError, match="different shape"):
            FrameRenderer().prepare_static_plot(
    ...
    def test_no_warnings_returns_no_layout_config(self) -> None:
        ...
        height, width, layout_config, warnings = FrameRenderer().prepare_static_plot(
            DEFAULT_SIC_SPEC, ground_truth, prediction
        )
    ...
    def test_warnings_return_a_layout_config_with_extra_title_space(self) -> None:
        ...
        _height, _width, layout_config, warnings = FrameRenderer().prepare_static_plot(
            DEFAULT_SIC_SPEC, ground_truth, prediction
        )
...
class TestPrepareDifference:
    def test_include_difference_false_returns_none(self) -> None:
        ...
        difference, colour_scale = FrameRenderer().prepare_difference(
            spec, ground_truth, prediction
        )
...
class TestDrawMainPanels:
    def test_levels_override_is_used_for_both_panels(self) -> None:
        ...
        image_gt, image_pred = FrameRenderer()._draw_main_panels(
            list(axs),
            ground_truth,
            prediction,
            DEFAULT_SIC_SPEC,
...
class TestDrawFrame:
    def test_requires_diff_colour_scale_when_including_difference(
        ...
        with pytest.raises(InvalidArrayError, match="diff_colour_scale"):
            FrameRenderer().draw_frame(
                list(axs), ground_truth, prediction, spec, no_land_mask
            )
        plt.close(fig)
    ...
    def test_signed_difference_uses_two_slope_norm(
        ...
        _, _, image_difference, _ = FrameRenderer().draw_frame(
            list(axs),
            ground_truth,
            prediction,
            spec,
